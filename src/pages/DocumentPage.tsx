...
const DocumentPage: React.FC = () => {
  ...
  const handleDocumentUpload = (option: 'gallery' | 'camera') => {
    setUploadModalVisible(false);

    const options = {
      mediaType: 'mixed', // You can use 'mixed' for both images and documents
      quality: 1,
      includeBase64: false,
    };
    ...
    if (option === 'gallery') {
      launchImageLibrary(options, (response) => {
        if (response.didCancel) {
          ...
          console.log('DocumentPicker Error: ', response.errorMessage);
        } else {
          Alert.alert('Document Selected', 'Document successfully selected from gallery.');
          // Handle the selected document here (e.g., display it or upload it)
        }
      });
      ...
      launchCamera(options, (response) => {
        if (response.didCancel) {
          ...
          Alert.alert('Document Captured', 'Document successfully captured.');
          // Handle the captured document here (e.g., display it or upload it)
        }
      });
    }
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={[styles.headerTitle, { flex: 1, textAlign: 'center' }]}>Documents</Text>
      </View>
        ...
        <View style={styles.uploadModalContainer}>
          <View style={styles.uploadModalView}>
            <TouchableOpacity
              ...
              onPress={() => handleDocumentUpload('gallery')}
            >
            ...
            >
              <Text style={styles.uploadModalButtonText}>Capture a document</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.uploadModalButton}
